import discord, aiohttp, json, asyncio, sqlite3, os, re, calendar
...
TICKET_CATEGORY_PESEZA = 1524072801085161654  # ⚔️ 페세자 파티 (0이면 자동 생성 — 실제 카테고리 ID로 교체 권장)
TICKET_CATEGORY_ABSENCE = 1524024689704435724  # 장기미접 티켓 카테고리 ID (0이면 자동 생성)

# 2-1. 치지직 방송 알림 (channelId: 표시 이름)
CHZZK_CHANNELS = {
    "43c0d0f534e9caf7713da151da591bfe": "만두집아들",
    "594ac8d5b7fde9faed2d1b20633fdac2": "에그밥",
}
CHZZK_CHECK_INTERVAL = 60  # 초 단위 상태 확인 주기
...
def schedule_ephemeral_delete(interaction: discord.Interaction, delay: int = 60):
    ...
    bot.loop.create_task(_delete_later())
...
async def get_or_create_category(guild: discord.Guild, ticket_type: str):
    mapping = {
        "join": TICKET_CATEGORY_JOIN, "qna": TICKET_CATEGORY_QNA, "comp": TICKET_CATEGORY_REPORT,
        "anon": TICKET_CATEGORY_ANON, "ato": TICKET_CATEGORY_ATO, "shrine": TICKET_CATEGORY_SHRINE, 
        "blood": TICKET_CATEGORY_BLOOD, "peseza": TICKET_CATEGORY_PESEZA
    }
...
async def send_rift_dm_all(spawn_time: datetime):
    """어둠의 틈 출현 하루 전날 00:00 KST에 전 서버원에게 DM 발송"""
    now = datetime.now(KST)
    # 젠 하루 전날 오전 9시에 알림 (예: 젠 시각이 7월 10일이면 7월 9일 09:00 알림)
    notify_time = (spawn_time - timedelta(days=1)).replace(hour=18, minute=0, second=0, microsecond=0)
    wait_sec = (notify_time - now).total_seconds()
    if wait_sec < 0:
        # 이미 지난 경우 1시간 전 알림으로 대체
        notify_time = spawn_time - timedelta(hours=1)
        wait_sec = (notify_time - now).total_seconds()
    if wait_sec < 0:
        return  # 이미 너무 늦음
    await asyncio.sleep(wait_sec)
    embed = discord.Embed(
        title="🌑 어둠의 틈 출현 D-1 알림",
        description=f"내일 어둠의 틈이 출현 예정입니다!\n\n⏰ 출현 예정 시각: **{spawn_time.strftime('%m/%d(%a) %H:%M')}**\n미리 준비하세요!",
        color=0x2c2f33
    )
...
async def register_rift_and_notify(interaction: discord.Interaction, kill_time: datetime):
    """어둠의 틈 처치 시간 DB 저장 + 채널 공지 + 하루 전 DM 예약"""
    spawn_time = kill_time + timedelta(days=5)
    conn = sqlite3.connect('bdo_data.db')
    conn.execute("INSERT OR REPLACE INTO rift_history (id, reporter_id, reporter_name, kill_time) VALUES (1, ?, ?, ?)",
    ...
    # 하루 전 DM 예약 (백그라운드 태스크)
    bot.loop.create_task(send_rift_dm_all(spawn_time))

class DarkRiftActionView(discord.ui.View):
    def __init__(self): super().__init__(timeout=None)
    
    @discord.ui.button(label="현재 시간으로 등록", style=discord.ButtonStyle.success, custom_id="rift_register_now_btn")
    async def bn(self, interaction: discord.Interaction, button: discord.ui.Button):
        now = datetime.now(KST)
        await interaction.response.send_message("✅ 어둠의 틈 처치 시간이 갱신(등록)되었습니다! 하루 전 전체 DM 알림이 예약됩니다.", ephemeral=True)
        schedule_ephemeral_delete(interaction)
        await register_rift_and_notify(interaction, now)
        
    @discord.ui.button(label="시간 직접 입력", style=discord.ButtonStyle.secondary, custom_id="rift_register_manual_btn")
    async def bm(self, i: discord.Interaction, b: discord.ui.Button):
        class M(discord.ui.Modal, title='시간 입력'):
            d=discord.ui.TextInput(label='일', required=True)
            h=discord.ui.TextInput(label='시', required=True)
            m=discord.ui.TextInput(label='분', required=True)
            
            async def on_submit(self, interaction: discord.Interaction):
                try: 
                    kt = datetime.now(KST).replace(day=int(self.d.value), hour=int(self.h.value), minute=int(self.m.value), second=0, microsecond=0)
                except ValueError: 
                ...
                await interaction.response.send_message("✅ 어둠의 틈 처치 시간이 갱신(등록)되었습니다! 하루 전 전체 DM 알림이 예약됩니다.", ephemeral=True)
                schedule_ephemeral_delete(interaction)
                await register_rift_and_notify(interaction, kt)
...
class StatusViewPanel(discord.ui.View):
    def __init__(self, s_type, s_title):
        ...
        if s_type == "edana":
            del_btn = discord.ui.Button(label="내 제보 삭제", style=discord.ButtonStyle.danger, custom_id="edana_report_delete_btn")
            del_btn.callback = self._delete_report
    ...
    async def _delete_report(self, interaction: discord.Interaction):
        ...
        if not rows:
            await interaction.response.send_message("삭제할 본인의 제보가 없습니다.", ephemeral=True); schedule_ephemeral_delete(interaction); return
        if len(rows) == 1:
            _delete_status_report(rows[0][0])
            await interaction.response.send_message("제보를 삭제했습니다.", ephemeral=True); schedule_ephemeral_delete(interaction)
            return
...
class CloseTicketModal(discord.ui.Modal, title="티켓 종료"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        # topic에서 creator_id 파싱 (파티 티켓: "app_id:N|user_id:N" / 일반 티켓: 숫자)
        creator_id = None
        app_id = None
        ...
        if "app_id:" in topic:
            ...
            try:
                ...
                app_id = int(parts.get("app_id", 0))
                creator_id = int(parts.get("user_id", 0))
            except Exception: pass
        elif topic.isdigit():
            creator_id = int(topic)
        else:
            try:
                async for msg in interaction.channel.history(limit=10, oldest_first=True):
                    if msg.mentions:
                        creator_id = msg.mentions[0].id; break
            except: pass
        ...
        if app_id:
            try:
                conn = sqlite3.connect('bdo_data.db')
                conn.execute("DELETE FROM party_applications WHERE id=?", (app_id,))
                conn.commit(); conn.close()
                # 패널 임베드 실시간 업데이트
...
class AbsenceModal(discord.ui.Modal, title='장기미접 사유 신고'):
    period = discord.ui.TextInput(
        label='미접 예정 기간',
        placeholder='예: 2026-07-10 ~ 2026-07-20',
        required=True,
        max_length=50
    )
    reason = discord.ui.TextInput(
        label='미접 사유',
        style=discord.TextStyle.paragraph,
        placeholder='연장 최대일수 30일까지 가능합니다.',
        required=True,
        max_length=1000
    )
    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True); schedule_ephemeral_delete(interaction)
        cat = await SetupJoinView._get_ticket_category(interaction.guild, TICKET_CATEGORY_ABSENCE)
        ch_name = f"미접-{interaction.user.name}-{datetime.now(KST).strftime('%m%d%H%M')}"
        ch = await cat.create_text_channel(name=ch_name, topic=str(interaction.user.id))
        embed = discord.Embed(title="📋 장기미접 사유 신고", color=0xe67e22)
        embed.add_field(name="신청자", value=interaction.user.mention, inline=False)
        embed.add_field(name="미접 예정 기간", value=self.period.value, inline=False)
        embed.add_field(name="미접 사유", value=self.reason.value, inline=False)
        embed.set_footer(text=f"신고 시각: {datetime.now(KST).strftime('%Y-%m-%d %H:%M')}")
        await ch.send(content=interaction.user.mention, embed=embed, view=CloseTicketView())
        await interaction.followup.send(f"✅ 미접 사유가 접수되었습니다: {ch.mention}", ephemeral=True)

class AbsenceTicketView(discord.ui.View):
    def __init__(self): super().__init__(timeout=None)
    @discord.ui.button(label="📋 장기미접 사유 신고", style=discord.ButtonStyle.primary, custom_id="ticket_absence")
    async def btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(AbsenceModal())

PARTY_NAMES = {"ato": "아토락시온", "shrine": "검은사당", "blood": "피의제단", "peseza": "페세자"}
PARTY_EMOJI = {"ato": "🏰", "shrine": "⛩️", "blood": "🩸", "peseza": "👑"}

PARTY_REQUIRED = {"ato": 5, "shrine": 5, "blood": 3, "peseza": 5}
...
class PartyCompleteModal(discord.ui.Modal, title='파티 완료 등록'):
    def __init__(self, default_kind_label: str, default_names: str):
        ...
        self.add_item(self.item_input); self.add_item(self.names_input); self.add_item(self.date_input); self.add_item(self.time_input)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            now = datetime.now(KST)
            year = now.year
            # 연말/연초 경계에서 다음 해로 넘어가는 일정을 자연스럽게 처리
            if mm < now.month - 6:
                year += 1
            datetime(year, mm, dd)  # 유효성 검사 (잘못된 날짜면 예외 발생)
...
class CloseGroupChannelView(discord.ui.View):
    ...
    def _parse_topic(topic: str):
        # topic 형식: "party_group:{kind}:{uid1},{uid2},..."
        try:
...
async def create_party_group_channel(guild: discord.Guild, kind: str, user_ids: list, user_names: list):
    cat_id = {"ato": TICKET_CATEGORY_ATO, "shrine": TICKET_CATEGORY_SHRINE, "blood": TICKET_CATEGORY_BLOOD, "peseza": TICKET_CATEGORY_PESEZA}[kind]
    cat = await SetupJoinView._get_ticket_category(guild, cat_id)
    names = {"ato": "아토락시온", "shrine": "검은사당", "blood": "피의제단", "peseza": "페세자"}
    
    # 권한 설정: 기본 유저(everyone)는 보기 금지, 봇과 관리자, 그리고 매칭된 '당사자'들만 채널 보기/쓰기 허용
    overwrites = {
    ...
    ch_name = f"그룹-{names[kind]}-{datetime.now(KST).strftime('%m%d%H%M')}"
    ...
    try:
        ...
        embed = discord.Embed(title=f"🎉 {names[kind]} 파티 매칭 완료!", color=0x2ecc71)
        embed.description = "파티 인원이 모두 모여 전용 채널이 생성되었습니다.\n이 채널은 **참여자와 관리자**만 볼 수 있습니다.\n일정 조율이 끝나면 **완료** 버튼으로 캘린더에 일정을 등록하고, 그 후 **파티 채널 닫기** 버튼으로 채널을 정리해주세요."
...
def build_party_embed() -> discord.Embed:
    ...
    for kind, name in PARTY_NAMES.items():
        ...
        if rows:
            ...
            for i, (uname, job, stats, timing) in enumerate(rows, 1):
                lines.append(f"`{i}.` {uname} │ {job} │ {stats} │ {timing}")
...
def _split_into_field_chunks(lines, limit=1024):
    """embed 필드 값 글자수 제한(1024자)에 맞춰 줄 목록을 여러 청크로 나눈다."""
    chunks = []
    current = ""
    for line in lines:
...
class CalendarDeleteSelect(discord.ui.Select):
    def __init__(self, rows):
        options = []
        # 디스코드 드롭다운은 최대 25개까지만 표시 가능합니다.
        for r_id, kind, parts, edate, etime in rows[:25]:
            # edate에서 'YYYY-MM-DD' 중 'MM-DD'만 자르기
            short_date = edate[5:] 
            label = f"[{short_date} {etime}] {kind}"
            # 설명란에 참여자 닉네임을 보여줍니다 (글자수 제한 50자)
...
class PartyCalendarView(discord.ui.View):
    ...
    async def p_m(self, i: discord.Interaction, b: discord.ui.Button):
        y, m = self._parse_year_month(i); y, m = (y, m - 1) if m > 1 else (y - 1, 12)
    ...
    async def n_m(self, i: discord.Interaction, b: discord.ui.Button):
        y, m = self._parse_year_month(i); y, m = (y, m + 1) if m < 12 else (y + 1, 1)
        await i.response.edit_message(embed=build_calendar_embed(i.guild.id, y, m))
    ...
    async def del_btn(self, i: discord.Interaction, b: discord.ui.Button):
        # 삭제 버튼은 관리자(또는 참모)만 누를 수 있도록 권한 체크
        ...
        # 현재 보고 있는 달(month)의 일정만 DB에서 불러옵니다.
        conn = sqlite3.connect('bdo_data.db')
        rows = conn.execute(
...
class PartyTicketView(discord.ui.View):
    ...
    async def peseza_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(self.PartyModal("peseza"))
    ...
    class PartyModal(discord.ui.Modal):
        ...
        async def on_submit(self, interaction: discord.Interaction):
            await interaction.response.defer(ephemeral=True); schedule_ephemeral_delete(interaction)
            cat_id = {"ato": TICKET_CATEGORY_ATO, "shrine": TICKET_CATEGORY_SHRINE, "blood": TICKET_CATEGORY_BLOOD, "peseza": TICKET_CATEGORY_PESEZA}[self.kind]
            ...
            c.execute(
                ...
                (self.kind, interaction.user.id, interaction.user.display_name,
                 self.job.value, self.stats.value, self.timing.value)
            )
            app_id = c.lastrowid
            conn.commit()

            # 인원 충족 여부 체크
            c.execute("SELECT id, user_id, user_name FROM party_applications WHERE kind=? ORDER BY created_at ASC", (self.kind,))
            apps = c.fetchall()
            ...
            if len(apps) >= required:
                ...
                embed = discord.Embed(title=f"{names[self.kind]} 파티 신청 완료", color=0x9b59b6)
                embed.add_field(name="신청자", value=interaction.user.mention, inline=False)
                ...
                await interaction.followup.send(f"✅ 티켓이 생성되었습니다: {ch.mention}\n🎉 **파티 인원이 모두 충족되어 그룹 전용 채널이 생성되었습니다!**", ephemeral=True)
            else:
                # 대기 상태
                conn.close()
...
class StatusKindSelectView(discord.ui.View):
    ...
    def _make_callback(self, kind):
        async def callback(interaction: discord.Interaction):
            s_title = STATUS_TITLES[self.s_type]
            if self.s_type == "edana":
                # 에다니아는 채널 통합 → 서버 선택 없이 바로 시간 선택
                embed = discord.Embed(title=f"{s_title} 제보", description=f"**종류:** {kind}\n\n남은 시간을 선택해주세요.", color=0x2b2d31)
                await interaction.response.edit_message(embed=embed, view=StatusTimeSelectView(self.s_type, kind, "통합"))
            else:
...
class BossDaySelectView(discord.ui.View):
    ...
    def __init__(self):
        super().__init__(timeout=None)  # 1. timeout=None 으로 수정됨
        today_str = WEEKDAYS[datetime.now(KST).weekday()][0]
        options = [
            discord.SelectOption(label=f"{day}요일{' (오늘)' if day == today_str else ''}", value=day)
            for day in ["월", "화", "수", "목", "금", "토", "일"]
        ]
        # 2. custom_id="boss_day_select_dropdown" 추가됨
        self.select = discord.ui.Select(placeholder="요일을 선택해주세요", options=options, custom_id="boss_day_select_dropdown")
        self.select.callback = self.select_callback
        self.add_item(self.select)
...
_chzzk_live_state: dict[str, bool] = {cid: False for cid in CHZZK_CHANNELS}

async def _fetch_chzzk_status(session: aiohttp.ClientSession, channel_id: str):
    """치지직 라이브 상태(OPEN/CLOSE) + 상세 정보를 조회한다."""
    ...
    try:
        async with session.get(
            ...
            return content.get("status") == "OPEN", content
    except Exception as e:
        print(f"⚠️ 치지직 상태 조회 실패 ({channel_id}): {e}")
        return None, None

@tasks.loop(seconds=CHZZK_CHECK_INTERVAL)
async def chzzk_live_alert_loop():
    """등록된 치지직 방송국이 방송을 켜면 전 서버원에게 개인 DM으로 알림"""
    async with aiohttp.ClientSession() as session:
        for channel_id, streamer_name in CHZZK_CHANNELS.items():
            is_live, content = await _fetch_chzzk_status(session, channel_id)
            if is_live is None:
                continue
            was_live = _chzzk_live_state.get(channel_id, False)
            if is_live and not was_live:
                title = (content or {}).get("liveTitle") or "방송이 시작되었습니다!"
                thumb = (content or {}).get("liveImageUrl")
                if thumb:
                    thumb = thumb.replace("{type}", "480")
                embed = discord.Embed(
                    title=f"🔴 {streamer_name} 방송 시작!",
                    description=f"**{title}**\n\n👉 [방송 바로가기](https://chzzk.naver.com/{channel_id})",
                    color=0x00e5a0
                )
                if thumb:
                ...
                for guild in bot.guilds:
                    for member in guild.members:
                        ...
                        try:
                            ...
                            pass
            _chzzk_live_state[channel_id] = bool(is_live)
...
async def on_ready():
    ...
    if not chzzk_live_alert_loop.is_running(): chzzk_live_alert_loop.start()
...
class UtilContainer(discord.ui.Container):
    ...
    async def _dekia(self, interaction: discord.Interaction):
        ...
        rk = sorted([{"n":it["name"], "l":it["light"], "p":p, "s":s, "u":p//it["light"]} for it, (p,s,_) in zip(DEKIA_DB, res) if p>0], key=lambda x:x["u"])
        d = "".join([f"**{idx+1}. {r['n']}**\n불빛: {r['l']} | 가격: {r['p']:,}\n**1개당: `{r['u']:,}`**\n\n" for idx, r in enumerate(rk)])
        msg = await interaction.followup.send(embed=discord.Embed(title="데키아 랭킹", description=d[:4000] or "데이터 없음", color=0xf1c40f), ephemeral=True, wait=True)
        bot.loop.create_task(_delete_message_after(msg, 60))
    ...
    async def _bless(self, interaction: discord.Interaction):
        ...
        rk = sorted([{"n":it["name"], "r":it["residue"], "p":p, "s":s, "u":p//it["residue"]} for it, (p,s,_) in zip(BLESS_DB, res) if p>0], key=lambda x:x["u"])
        d = "".join([f"**{idx+1}. {r['n']}**\n잔재: {r['r']} | 가격: {r['p']:,}\n**1개당: `{r['u']:,}`**\n\n" for idx, r in enumerate(rk)])
        msg = await interaction.followup.send(embed=discord.Embed(title="영물 랭킹", description=d[:4000] or "데이터 없음", color=0x2ecc71), ephemeral=True, wait=True)
        bot.loop.create_task(_delete_message_after(msg, 60))
...
@bot.tree.command(name="설치-장기미접", description="[관리자] 장기미접 사유 신고 패널 설치")
async def cmd_install_absence(interaction: discord.Interaction):
    if not getattr(interaction.user.guild_permissions, 'administrator', False) and not interaction.user.get_role(ADMIN_ROLE_ID):
        await interaction.response.send_message("관리자만 사용할 수 있습니다.", ephemeral=True); schedule_ephemeral_delete(interaction); return
    embed = discord.Embed(
        title="📋 장기미접 사유 신고",
    ...
    await interaction.channel.send(embed=embed, view=AbsenceTicketView())
    await interaction.response.send_message("✅ 장기미접 패널 설치 완료", ephemeral=True); schedule_ephemeral_delete(interaction)
...
async def cmd_install_party(interaction: discord.Interaction):
    ...
    # 패널 메시지 ID 저장 (신청/취소 시 실시간 업데이트에 사용)
    conn = sqlite3.connect('bdo_data.db')
    conn.execute("INSERT OR REPLACE INTO party_panel (guild_id, channel_id, message_id) VALUES (?,?,?)",
...
@bot.tree.command(name="설치-파티캘린더", description="[관리자] 파티 완료 일정 캘린더 패널 설치")
async def cmd_install_party_calendar(interaction: discord.Interaction):
    if not getattr(interaction.user.guild_permissions, 'administrator', False) and not interaction.user.get_role(ADMIN_ROLE_ID):
        await interaction.response.send_message("관리자만 사용할 수 있습니다.", ephemeral=True); schedule_ephemeral_delete(interaction); return
    now = datetime.now(KST)
    ...
    conn = sqlite3.connect('bdo_data.db')
    conn.execute("INSERT OR REPLACE INTO party_calendar_panel (guild_id, channel_id, message_id) VALUES (?,?,?)",
                 (interaction.guild.id, interaction.channel.id, panel_msg.id))
    conn.commit(); conn.close()
    await interaction.response.send_message("✅ 파티캘린더 패널 설치 완료", ephemeral=True); schedule_ephemeral_delete(interaction)
...
async def setup_hook():
    ...
    bot.add_view(AbsenceTicketView())
    bot.add_view(QnaTicketView())
    bot.add_view(CouponPanelView())
    bot.add_view(UtilView())
    bot.add_view(SetupBossView())
    bot.add_view(BossDaySelectView())  # 변경됨 (기존 BossTimeSelectView -> BossDaySelectView)
    bot.add_view(CloseTicketView())
    bot.add_view(CloseGroupChannelView())  # 새로 생성되는 파티 그룹 닫기/완료 버튼 뷰 추가
    bot.add_view(PartyCalendarView())  # 파티 완료 일정 캘린더 패널 (이전달/오늘/다음달 버튼)
    ...
    bot.add_view(DarkRiftActionView())  # 어둠의 틈 버튼 영구 지속 추가
