...
def init_db():
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS party_panel (
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS party_calendar_panel (
        guild_id INTEGER PRIMARY KEY,
        channel_id INTEGER NOT NULL,
        message_id INTEGER NOT NULL
    )''')
...
class PartyCalendarView(discord.ui.View):
    ...
    async def prev_month(self, interaction: discord.Interaction, button: discord.ui.Button):
        year, month = self._parse_year_month(interaction)
        year, month = (year, month - 1) if month > 1 else (year - 1, 12)
        await interaction.response.edit_message(embed=build_calendar_embed(interaction.guild.id, year, month))
    ...
    async def today(self, interaction: discord.Interaction, button: discord.ui.Button):
        now = datetime.now(KST)
        await interaction.response.edit_message(embed=build_calendar_embed(interaction.guild.id, now.year, now.month))
...
async def update_party_calendar_panel(guild: discord.Guild):
    """party_calendar_panel 테이블에 저장된 패널을 이번 달 기준으로 갱신한다."""
    conn = sqlite3.connect('bdo_data.db')
