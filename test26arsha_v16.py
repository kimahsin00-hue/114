...
TICKET_CATEGORY_PESEZA = 0  # ⚔️ 페세자 파티 (0이면 자동 생성 — 실제 카테고리 ID로 교체 권장)
TICKET_CATEGORY_ABSENCE = 0  # 장기미접 티켓 카테고리 ID (0이면 자동 생성)
...
async def send_rift_dm_all(spawn_time: datetime):
    ...
    notify_time = (spawn_time - timedelta(days=1)).replace(hour=9, minute=0, second=0, microsecond=0)
...
class DarkRiftActionView(discord.ui.View):
    ...
    async def bn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        conn.close()
        now = datetime.now(KST)
        await interaction.response.send_message("✅ 어둠의 틈 처치 시간이 등록되었습니다! 하루 전 전체 DM 알림이 예약됩니다.", ephemeral=True); schedule_ephemeral_delete(interaction)
    ...
    async def bm(self, i: discord.Interaction, b: discord.ui.Button):
        ...
        class M(discord.ui.Modal, title='시간 입력'):
            ...
            async def on_submit(self, interaction: discord.Interaction):
                ...
                await interaction.response.send_message("✅ 어둠의 틈 등록됨! 하루 전 전체 DM 알림이 예약됩니다.", ephemeral=True); schedule_ephemeral_delete(interaction)
...
class StatusDeleteSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message("🗑️ 제보를 삭제했습니다.", ephemeral=True)
...
class StatusViewPanel(discord.ui.View):
    ...
    async def _delete_report(self, interaction: discord.Interaction):
        ...
        if len(rows) == 1:
            ...
            await interaction.response.send_message("🗑️ 제보를 삭제했습니다.", ephemeral=True); schedule_ephemeral_delete(interaction)
            return
...
class AbsenceModal(discord.ui.Modal, title='장기미접 사유 신고'):
    ...
    reason = discord.ui.TextInput(
        label='미접 사유',
        style=discord.TextStyle.paragraph,
        placeholder='예: 군 입소 / 해외여행 / 개인 사정 등 자세히 적어주세요.',
        required=True,
        max_length=1000
    )
    async def on_submit(self, interaction: discord.Interaction):
