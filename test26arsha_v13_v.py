...
class PartyTicketView(discord.ui.View):
    ...
    # ── Row 1: 현황 보기 버튼
    @discord.ui.button(label="아토락시온 현황", style=discord.ButtonStyle.secondary, custom_id="party_status_ato", row=1)
    async def status_ato(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self._show_status(interaction, "ato", "아토락시온")

    @discord.ui.button(label="검은사당 현황", style=discord.ButtonStyle.secondary, custom_id="party_status_shrine", row=1)
    async def status_shrine(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self._show_status(interaction, "shrine", "검은사당")

    @discord.ui.button(label="피의제단 현황", style=discord.ButtonStyle.secondary, custom_id="party_status_blood", row=1)
    async def status_blood(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self._show_status(interaction, "blood", "피의제단")

    @staticmethod
    async def _show_status(interaction: discord.Interaction, kind: str, name: str):
        conn = sqlite3.connect('bdo_data.db')
        rows = conn.execute(
            "SELECT user_name, job, stats, timing, created_at FROM party_applications WHERE kind=? ORDER BY created_at ASC",
            (kind,)
        ).fetchall()
        conn.close()

        embed = discord.Embed(title=f"⚔️ {name} 파티 신청 현황", color=0x9b59b6)
        if not rows:
            embed.description = "현재 신청자가 없습니다."
        else:
            embed.description = f"총 **{len(rows)}명** 신청 중\n"
            for i, (uname, job, stats, timing, created_at) in enumerate(rows, 1):
                date_str = created_at[:10] if created_at else "-"
                embed.add_field(
                    name=f"{i}. {uname}  |  {date_str}",
                    value=f"**직업:** {job}\n**공/방합계:** {stats}\n**희망 시간대:** {timing}",
                    inline=False
                )
        await interaction.response.send_message(embed=embed, ephemeral=True)
